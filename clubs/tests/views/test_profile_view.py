...
class ProfileViewTestCase(TestCase):
    ...
    def setUp(self):
        self.url = reverse('profile', kwargs={"club_id": 0, "user_id": 1})

    def test_profile_url(self):
        self.assertEqual(self.url, '/profile/0/1')

    def test_non_logged_in_user_gets_user_profile_page(self):
        response = self.client.get(self.url, follow=True)
        expected_url = '/log_in/?next=/profile/0/1'
        self.assertRedirects(response, expected_url)
        self.assertTemplateUsed(response, 'log_in.html')

    def test_applicant_user_not_in_any_club_gets_own_profile_page(self):
    ...
    def test_applicant_user_in_1_club_gets_own_profile_page(self):
        pass
        
    def test_member_user_gets_own_profile_page(self):
        user = User.objects.get(id=1)
        url = reverse('profile', kwargs={"club_id": 0,"user_id": 1})
    ...
    def test_member_user_gets_applicant_user_profile_page(self):
        self.client.login(email='janedoe@example.org', password='Password123')
        url = reverse('profile', kwargs={"club_id": 0,"user_id": 0})
        response = self.client.get(url, follow=True)
    ...
    def test_member_user_gets_other_member_profile_page(self):
        self.client.login(email='janedoe@example.org', password='Password123')
        url = reverse('profile', kwargs={"club_id": 0,"user_id": 2})
        response = self.client.get(url, follow=True)
        expected_url = reverse('profile', kwargs={"user_id": 1})
        self.assertRedirects(response, expected_url)
        self.assertTemplateUsed(response, 'profile.html')
